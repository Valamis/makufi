...
<#if layout??>
    ...
    <#assign canonicalUrl = portalUtil.getCanonicalURL(theme_display.getPortalURL() + theme_display.getURLCurrent(), theme_display, layout) />
</#if>
...
<#-- ---------- Search ---------- -->
<#assign searchScope = "everything" />
<#if theme_display.getScopeGroup().getExpandoBridge().hasAttribute("Search-Scope-Everything")>
	<#attempt>
		<#if theme_display.getScopeGroup().getExpandoBridge().getAttribute("Search-Scope-Everything")>
			<#assign searchScope = "everything" />
		<#else>
			<#assign searchScope = "" />
		</#if>
...
<#if layout??>
	...
	<#if page.getType()?matches('portlet')>
...
<#if the_title ?has_content && !company_name?matches(site_name) && !page_group.isLayoutPrototype()>
